...
export class ListCommand extends BaseCommand {
  ...
  async execute(options: ListOptions): Promise<void> {
    ...
    try {
      ...
      if (!localConfig?.projectId) {
        ...
        this.logger.error('Please run "jx link" first');        process.exit(1);
      }
  ...
  private getMockDeployments(limit: number): Deployment[] {    const deployments: Deployment[] = [];
    
    for (let i = 0; i < limit; i++) {
      deployments.push({
        id: `dpl_${Math.random().toString(36).substr(2, 9)}`,
        url: `https://myapp-${Math.random().toString(36).substr(2, 5)}.jx.app`,
