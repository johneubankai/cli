...
export class VercelLinkCommand extends SubCommand {
  parentCommand = 'vercel';
  name = 'link';
  description = 'Link a project to your Vercel account';
  ...
  async execute(options: VercelLinkOptions): Promise<void> {
    ...
    try {
        ...
        const confirm = await inquirer.prompt([
          {
            type: 'confirm',
            name: 'proceed',
            message: `Link "${projectName}" to your Vercel account?`,
            default: true,
          },
        ]);
      ...
      this.logger.info(`Project ID: ${config.projectId}`);
      this.logger.info(`Framework: ${framework || 'None detected'}`);
    } catch (error) {
