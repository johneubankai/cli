...
export class VercelLogsCommand extends SubCommand {
  ...
  async execute(options: VercelLogsOptions, command?: Command): Promise<void> {
    ...
    try {
      ...
      if (!deploymentUrl) {
        ...
        this.logger.error('Usage: jx vercel logs <deployment-url>');
        this.logger.info('Run "jx vercel list" to see your deployments');
        process.exit(1);
      }
