...
interface VercelEnvRemoveOptions {
  yes?: boolean;
}

export class VercelEnvRemoveCommand extends SubCommand {
  ...
  async execute(options: VercelEnvRemoveOptions, command?: Command): Promise<void> {
    ...
    try {
      ...
      spinner.succeed(`Environment variable "${key}" removed successfully!`);
    } catch (error) {
