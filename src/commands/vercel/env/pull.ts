...
interface VercelEnvPullOptions {
  production?: boolean;
  preview?: boolean;
  development?: boolean;
  output?: string;
...
export class VercelEnvPullCommand extends SubCommand {
  ...
  async execute(options: VercelEnvPullOptions, command?: Command): Promise<void> {
    const args = command?.args || [];
    const outputFile = args[0] || '.env';
